	...
	static class FerrisWheel
	{
		...
		private static async Task ControlloPlayer()
		{
			if (Game.PlayerPed.IsInRangeOf(new Vector3(-1661.95f, -1127.011f, 12.6973f), 20f))
			{
				...
				{
					HUD.ShowHelp("Press ~INPUT_CONTEXT~ to get on the Wheel");
					if (Input.IsControlJustPressed(Control.Context))
					{
						HUD.ShowNotification("Wait.. The first free cabin is coming..");
			...
			if (!RideEnd)
			{
				...
				{
					HUD.ShowNotification("The wheel will stop once your cabin reaches ground to let you get off");
